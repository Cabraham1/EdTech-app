...
export function validateStudentInput(
  ...
  if (!isUpdate || input.registrationNumber !== undefined) {
    ...
    } else if (!VALIDATION_RULES.registrationNumber.pattern.test(input.registrationNumber)) {
      errors.push({
        field: 'registrationNumber',
  ...
  if (!isUpdate || input.dob !== undefined) {
    if (!input.dob || input.dob.trim().length === 0) {
      ...
      } else if (age < VALIDATION_RULES.age.min || (age === VALIDATION_RULES.age.min && monthDiff < 0)) {
        errors.push({
          field: 'dob',
  ...
  if (!isUpdate || input.gpa !== undefined) {
    if (input.gpa === undefined || input.gpa === null) {
      ...
      } else if (gpa < VALIDATION_RULES.gpa.min || gpa > VALIDATION_RULES.gpa.max) {
        errors.push({
          field: 'gpa',
